...
public class Task extends JFrame implements ActionListener {
	...
	public Task() {
		...
		*/
		...
		txtInterval.setBounds(119, 21, 33, 20);
		...
		lblByTabs = new JLabel("-MixPotion");
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if( evt.getActionCommand().equals( "Start" ) ) {
			try {
				Mix mix = new Mix( Task.this,
						//Integer.parseInt( "0" + txtN.getText().trim()),
						Integer.parseInt( "0" + txtInterval.getText().trim() ));
				
				JOptionPane.showMessageDialog( null, "Task will start in 3sec.", "Warning", JOptionPane.WARNING_MESSAGE );
				ExecutorService executor = Executors.newCachedThreadPool();
				executor.execute( mix );
